...
public class Sketchbook {
	...
	public Sketchbook(MainWindow editor) {
		...
		if (sketchbookPath == null) {
			 ...
			 * 
			 * if (Base.platform == Base.MACOSX) { // on macosx put the
			 * sketchbook in the "Documents" folder home = new File(home,
			 * "Documents");
			 *  } else if (Base.platform == Base.WINDOWS) { // on windows put
			 * the sketchbook in the "My Documents" folder home = new File(home,
			 * "My Documents"); }
			 */
	...
	/**
	 * Handle creating a sketch folder, return its base .gcode file or null if
	 * the operation was cancelled.
	 */
	public String handleNew(boolean noPrompt, boolean shift) throws IOException {
		File newbieDir = null;
		...
		if (prompt) {
			...
			FileDialog fd = new FileDialog(editor,
					"Create sketch folder named:", FileDialog.SAVE);
		...
		if (Base.isMacOS()) {
			MRJFileUtils.setFileTypeAndCreator(newbieFile, MRJOSType.kTypeTEXT,
					new MRJOSType("Gcde"));
	...
	/**
	 * Produce a sanitized name that fits our standards for likely to work. <p/>
	 ...
	 * partition in OS X and using a thumb drive. <p/> This helper function
	 * replaces everything but A-Z, a-z, and 0-9 with underscores. Also
	 * disallows starting the sketch name with a digit.
	 */
	...
	/**
	 * Asynchronous version of menu rebuild to be used on 'new' and 'save', to
	 * prevent the interface from locking up until the menus are done.
	 */
	public void rebuildMenusAsync() {
		// disabling the async option for actual release, this hasn't been
		// tested
		/*
	...
	/**
	 * Rebuild the menu full of sketches based on the contents of the
	 * sketchbook.
	 * 
	 * Creates a separate JMenu object for the popup, because it seems that
	 * after calling "getPopupMenu" the menu will disappear from its original
	 * location.
	 */
	...
	protected boolean addSketches(JMenu menu, File folder) throws IOException {
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (entry.exists()) {
				...
				// not a sketch folder, but maybe a subfolder containing
				// sketches
			...
			for (int j = 0; j < entries.length; j++) {
				...
				if (gcode.exists() && (Base.calcFolderSize(prey) == 0)) {
					...
					if (Base.preferences.getBoolean("sketchbook.auto_clean",true)) {
						...
						String prompt = "Remove empty sketch titled \""
								+ entries[j] + "\"?";
