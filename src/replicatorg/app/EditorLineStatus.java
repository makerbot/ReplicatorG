...
public class EditorLineStatus extends JComponent {
	...
	public void set(int currentLine, double elapsedTime, double timeRemaining) {
		...
		double percentage = Math
				.round(((double) currentLine / (double) total) * 10000.0) / 100.0;
		text = "Commands: ";
		...
		text += "     |     ";
		text += "Elapsed time: "
				+ EstimationDriver.getBuildTimeString(elapsedTime, true);
		text += "     |     ";
		text += "Time remaining: "
				+ EstimationDriver.getBuildTimeString(timeRemaining, true);
