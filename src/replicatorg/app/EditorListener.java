...
/**
 * Filters key events for tab expansion/indent/etc. <p/> For version 0099, some
 * changes have been made to make the indents smarter. There are still issues
 * though: + indent happens when it picks up a curly brace on the previous line,
 * but not if there's a blank line between them. + It also doesn't handle single
 * indent situations where a brace isn't used (i.e. an if statement or for loop
 * that's a single line). It shouldn't actually be using braces. Solving these
 * issues, however, would probably best be done by a smarter parser/formatter,
 * rather than continuing to hack this class.
 */
public class EditorListener {
	...
	/**
	 * Intercepts key pressed events for JEditTextArea. <p/> Called by
	 * JEditTextArea inside processKeyEvent(). Note that this won't intercept
	 * actual characters, because those are fired on keyTyped().
	 */
	public boolean keyPressed(KeyEvent event) {
		...
		case 9:
			if (tabsExpand) { // expand tabs
				...
				// get the number of braces, to determine whether this is an
				// indent
				int braceBalance = 0;
				int index = lineStart;
		...
		case 13:
			if (autoIndent) {
				...
				/*
				 * int offset = 0; int realIndex = origIndex; for (int i = 0; i <
				 * realIndex-1; i++) { if ((contents[i] == 13) && (contents[i+1] ==
				 * 10)) { offset++; realIndex++; } } // back up until \r \r\n or
				 * \n.. @#($* cross platform //System.out.println(origIndex + "
				 * offset = " + offset); origIndex += offset; // ARGH!#(*
				 * WINDOWS#@($*
				 */
				...
				// can cause trouble. for 0099, was ignoring the case, but this
				// is
				// annoying, so in 0122 we're trying to fix that.
				/*
				 * if (spaceCount - extraCount > 0) { spaceCount -= extraCount; }
				 */
				...
				if (braceCount > 0) {
					...
					// sel - tabSize will be -1 if start/end parens on the same
					// line
		...
		case '}':
			if (autoIndent) {
				 ...
				 * this isn't perfect cuz it'll pick up commented areas, // but
				 * that's not really a big deal and can be fixed when // this is
				 * all given a more complete (proper) solution. index =
				 * prevCharIndex; int braceDepth = 1; finished = false; while
				 * ((index != -1) && (!finished)) { if (contents[index] == '}') { //
				 * aww crap, this means we're one deeper // and will have to
				 * find one more extra { braceDepth++; index--; } else if
				 * (contents[index] == '{') { braceDepth--; if (braceDepth == 0) {
				 * finished = true; } // otherwise just teasing, keep going.. }
				 * else { index--; } } // never found a proper brace, be safe
				 * and don't do anything if (!finished) return false;
				 *  // check how many spaces on the line with the matching open
				 * brace int pairedSpaceCount = calcSpaceCount(index, contents);
				...
				textarea.setSelectedText(Editor.EMPTY.substring(0,
						pairedSpaceCount));
	 ...
	 * int code = event.getKeyCode();
	 * 
	 * if ((event.getModifiers() & CMD_ALT) == CMD_ALT) { if (code ==
	 * KeyEvent.VK_LEFT) { //if (c == '[') { editor.sketch.prevCode(); return
	 * true; } else if (code == KeyEvent.VK_RIGHT) { //} else if (c == ']') {
	 * editor.sketch.nextCode(); return true; } } return false; }
	 */
	...
	/**
	 * Walk back from 'index' until the brace that seems to be the beginning of
	 * the current block, and return the number of spaces found on that line.
	 */
	...
	/**
	 * Get the character array and blank out the commented areas. This hasn't
	 * yet been tested, the plan was to make auto-indent less gullible (it gets
	 * fooled by braces that are commented out).
	 */
