...
class BRRepository:
	'A class to handle the better raft settings.'
	def __init__(self):
		'Set the default settings, execute title & settings fileName.'
		skeinforge_profile.addListsToCraftTypeRepository('skeinforge_application.skeinforge_plugins.craft_plugins.betterraft.html', self )
		self.fileNameInput = settings.FileNameInput().getFromFileName(
			fabmetheus_interpret.getGNUTranslatorGcodeFileTypeTuples(), 'Open File for betterraft', self, '')
		self.openWikiManualHelpPage = settings.HelpPage().getOpenFromAbsolute('')
		self.activateBetterraft = settings.BooleanSetting().getFromValue('Activate Betterraft', self, True)
...
#		self.turnFanOffAtEnding = settings.BooleanSetting().getFromValue('Turn Fan Off at Ending', self, True)
		self.executeTitle = 'Betterraft'

	def execute(self):
		'Betterraft button has been clicked.'
